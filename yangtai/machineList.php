	...
	<body>
						...
						<div class="list hidden">
							<div class="item">
								...
								<div class="list-content">
									...
									<div class="addresscontent">
										<span class="left15">设备名称：<label class="beizhu"></label>
									</div>
		...
		<script>
			...
			if(!window.localStorage.getItem("isonline")) {
				window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxe9b057d10330ccf4&redirect_uri=http://www.ccsc58.cc/leng/yangtai/oauth2_templatform.php&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect";
			}else{
							...
							if(res.code == 0 && res.message == 'success') {
							   if(res.data.count<=5){
								...
								for(var i = 0; i < res.data.data.length; i++) {
									...
									_dem.find(".list-content .beizhu").html(res.data.data[i].beizhu==''?'':res.data.data[i].beizhu);
									...
									_dem.find(".list_tittle .main").parent().addClass('hidden');
							   ...
							   if(res.data.count<=5){
								...
								for(var i = 0; i < res.data.data.length; i++) {
									...
									if(res.data.data[i].shebeibianhao.substr(0, 1)=='2'&&res.data.data[i].shebeibianhao.length==7){
										...
										_dem.find(".list-content .beizhu").text(res.data.data[i].beizhu==''?'':res.data.data[i].beizhu);
...
//				$(".window_post").removeClass('hidden');
	...
	$("body").on('click','.list .left15' ,function(event) {
