$(document).ready(function () {
  ...
  $(".work_line").on("click", function () {
    ...
    } else if (spanval == "参数信息设置") {
      location.href = "../html/parameterSet.html?num_m=" + _num_ms;
    } else if (spanval == "设备打印设置") {
      location.href = "../html/printSet.html?num_m=" + _num_ms;
    } else if (spanval == "设备报警设置") {
      location.href = "../html/warningSet.html?num_m=" + _num_ms;
    } else if (spanval == "系统报警设置") {
      location.href = "../html/systemSet.html?num_m=" + _num_ms;
    } else {
      location.href = "../html/additionalSet.html?num_m=" + _num_ms;
    }
  });
  details_list_get();
  ...
  function details_list_get() {
    $(".header").html(
      '<div class="button_back_list"><img src="../img/back.png"><span></span></div>温控数据'
    );
    /*
     * 返回列表页的时间；
     */
  ...
  function details_now_map() {
    $(".details_now").on("click", function () {
      var _jingDu = $(this).find(".lng").text();
      var _weiDu = $(this).find(".lat").text();
      var _time = $(this).find(".worktime").text();
      var _wenDu01 = $(this).find(".temp1").text();
      var _wenDu02 = $(this).find(".temp2").text();
      var _shiDu = $(this).find(".humidity").text();
    ...
    function _details_map(_jingDu, _weiDu, _time, _wenDu01, _wenDu02, _shiDu) {
      ...
      function now_one_map(x, y) {
        ...
        if (_wenDu02 == "-") {
          ...
          strs =
            "<br/>温度01/温度02：" + _wenDu01 + "℃/" + _wenDu02 + "℃/" + "%RH<br/>时间：" + _time;
        }
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.status == 0) {
          ...
          _dem
            .find(".details_now_right .details_address span:nth-of-type(1)")
            .html(data.result[0].x);
          _dem
            .find(".details_now_right .details_address span:nth-of-type(2)")
            .html(data.result[0].y);
        } else {
  ...
  $.ajax({
    url: "http://erpapi.ccsc58.com/xiandun/public/index.php/index/device/xiandun_get_history_data",
    type: "post",
    data: {
      UserP: "w",
        ...
        if (_json.resultCode == "null") {
          ...
          for (var i = 0; i < _list.length; i++) {
            ...
            if (_json.model_type == "TT") {
              _demN
                .find(".list-content .temp2")
                .html(_list[i].temperature02 == 0 ? "-" : _list[i].temperature02 + "℃");
              _demN.find(".list-content .humidity").parents("p").addClass("hidden");
            } else {
              _demN
                .find(".list-content .humidity")
                .html(_list[i].humidity == "0.0" ? "-" : _list[i].humidity);
            ...
            if (_list[i].xinghaoqiangdu >= 0 && _list[i].xinghaoqiangdu < 5) {
              _demN.find(".list-content .signal").html("无信号");
            } else if (_list[i].xinghaoqiangdu >= 5 && _list[i].xinghaoqiangdu < 13) {
              _demN.find(".list-content .signal").html("弱");
            } else if (_list[i].xinghaoqiangdu >= 13 && _list[i].xinghaoqiangdu < 20) {
              _demN.find(".list-content .signal").html("良");
            } else if (_list[i].xinghaoqiangdu >= 20 && _list[i].xinghaoqiangdu < 26) {
              _demN.find(".list-content .signal").html("好");
            } else if (_list[i].xinghaoqiangdu >= 26) {
              _demN.find(".list-content .signal").html("强");
            } else {
              _demN.find(".list-content .signal").html("无信号");
            }
            _demN.find(".list-content .speed").html(_list[i].speed);
            _demN
              .find(".list-content .boxstate")
              .html(_list[i].xiangzistate == "close" ? "关闭" : "开启");
    ...
    error: function () {
      var _s =
        '<a href="../machineList.php">您查看的设备不存在或未开启，请点击返回前一页面重新查看</a>';
      var _dem = $(".details_now").clone().removeClass("hidden").html(_s);
      _dem.css({
        width: "100vw",
        height: "10rem",
  ...
  $(".fujiaset").on("click", function () {
    location.href = "../html/additionalSet.html";
  });
          ...
          for (var i = 0; i < _json.resultCode.length; i++) {
            ...
            if (_json.model_type == "TH") {
              _dem
                ...
                .html(
                  _json.resultCode[i].humidity == "0.0" ? "-" : _json.resultCode[i].humidity + "%"
                );
            } else {
              _dem
                .find("li:nth-of-type(5)")
                .html(
                  _json.resultCode[i].temperature02 == "0.0"
                    ? "-"
                    : _json.resultCode[i].temperature02 + "℃"
                );
            }
            _dem
              .find("li:nth-of-type(2)")
              .html(
                _json.resultCode[i].time
                  .replace(_json.resultCode[i].time.match(/^2[0-9]{3}/)[0] + "-", "")
                  .replace(_json.resultCode[i].time.match(/\s/)[0], "<br>")
              );
    ...
    if ($(this).index() == 1) {
      ...
      if (_down_panDuan == 0) {
        ...
        window.location.href =
          ...
          _num_m +
          "&start=" +
          $("#history_start_time").val() +
          "&end=" +
          $("#history_end_time").val();
      }
    } else {
      ...
      success: function (data) {
        ...
        if (_json.resultCode != "null") {
          var _jing = [],
            ...
            _timeCha = [];
          for (
            var j = 0, i = _json.resultCode.length - 1;
            j < _json.resultCode.length, i >= 0;
            j++, i--
          ) {
            _jing[j] = _json.resultCode[i].jingdu;
            if (_json.model_type == "TT") {
              _temp[j] =
                _json.resultCode[i].temperature01 + "℃/" + _json.resultCode[i].temperature02 + "℃/";
            } else {
              _temp[j] =
                _json.resultCode[i].temperature01 + "℃/" + _json.resultCode[i].humidity + "%";
            }
    ...
    function mao_map(_jing, _wei, _temp, _time, _timeCha, type) {
      ...
      function my_map_new(_jing_d, _wei_d, _tem, _tim, a, b, _timeC, type) {
        ...
        function my_add_map_new(_j, _w, _address, type) {
          ...
          if (type == "TT") {
            content =
              '时间:<span style="color:blue;font-size:12px;">' +
              _tim +
              '</span><br/>位置:<span style="color:blue;font-size:12px;">' +
              _address +
              '</span><br/>温度01/温度02：<span style="color:blue;font-size:12px;">' +
              _tem +
              '</span><br>停留时间：<span style="color:blue;font-size:12px;">' +
              parseInt(_timeC) +
              "分钟</span>";
          } else {
            content =
              ...
              '</span><br/>位置:<span style="color:blue;font-size:12px;">' +
              _address +
              '</span><br/>温度/湿度：<span style="color:blue;font-size:12px;">' +
              _tem +
              '</span><br>停留时间：<span style="color:blue;font-size:12px;">' +
              parseInt(_timeC) +
              "分钟</span>";
  ...
  function myWarning() {
    ...
    function warning_more(_warning_more, _flag) {
      ...
      $.ajax({
        url: "http://erpapi.ccsc58.com/xiandun/public/index.php/index/alarm/device_list",
        ...
        success: function (data) {
          ...
          if (_json.message == "nodata") {
            if (_flag == 0) {
              ...
              $(".more_warning").html(
                '<a href="../machineList.php">该设备未设置报警规格,点击返回列表页</a>'
              );
              $(".wait").addClass("hidden");
            } else {
            ...
            for (var i = 0; i < warning_data.length; i++) {
              //湿度阀值数据暂无；
              var _dem = $(".warning_list")
                .eq(0)
                .clone()
                .removeClass("hidden")
                .appendTo(".scroll_box_warning");
              if (
                warning_data[i].shebeibianhao.substr(0, 1) == "2" &&
                warning_data[i].shebeibianhao.length == 7
              ) {
                ...
                if (_json.model_type == "TH") {
                  ...
                  let shidulowstr =
                    warning_data[i].chaodishidubaojingfazhi == -99999.9
                      ? "-"
                      : warning_data[i].chaodishidubaojingfazhi;
                  let shiduhigtstr =
                    warning_data[i].chaogaoshidubaojingfazhi == 99999.9
                      ? "-"
                      : warning_data[i].chaogaoshidubaojingfazhi;
                  ...
                  let temp2lowstr =
                    warning_data[i].baojingwendu_two_xiaxian == -99999.9
                      ? "-"
                      : warning_data[i].baojingwendu_two_xiaxian;
                  let temp2higtstr =
                    warning_data[i].baojingwendu_two_shangxian == 99999.9
                      ? "-"
                      : warning_data[i].baojingwendu_two_shangxian;
                  _dem
                    .find(".list-content .temp2")
                    .html(warning_data[i].temperature02 == 0 ? "" : warning_data[i].temperature02);
                ...
                _dem
                  .find(".list-content .tempLimit")
                  .html(
                    warning_data[i].baojingwendu_xiaxian +
                      "~" +
                      warning_data[i].baojingwendu_shangxian
                  );

                _dem
                  .find(".list-content .armType")
                  .html(warning_data[i].baojingleixing == false ? "温度报警" : "");
              }
            }
            $(".wait").addClass("hidden");
          }
  ...
  function history_my_zhe(_history_start_time, _history_end_time, _length) {
    ...
    $.ajax({
      ...
      data: {
        ...
        EndTime: _history_end_time,
        StartNo: 0,
        Length: _length, //_length or 5000;
        admin_permit: "zjly8888",
        admin_user: _userName,
        admin_pass: _userPass,
      },
    ...
    function history_zhe(
      ...
      $("#history_content_zhe").highcharts({
        ...
        xAxis: {
          ...
          title: {
            text:
              _objtype.val1 + "此时间段最高温度" + sxxian[1] + "℃" + ",最低温度:" + sxxian[0] + "℃",
          },
          plotLines: [
            {
              color: "#ccc", //线的颜色
