		...
		$.ajax({
			...
			success: function(data) {
				...
				if(_json.message == "success" && _json.data != "") {
					...
					console.log(_json);
					var str='';
					$.each(_json.data, function(index,value) {
						var _state='';
						if(value.state==1){
							_state='未完成'
						}else{
						...
						let href="dingdan_xiang.html?danhao="+value.danhao;
...
//					$(".wait").addClass("hidden");
					/*订单详情入口点击事件；
					 */
					$("body").on("click",'.dingdan_content', function() {
						var _dingdan = $(this).find("p:nth-of-type(1) span").html();
					...
					$("body").on("click",".dingdan_content" ,function() {
						var _search_dan = $(this).find('.yundannum').text().replace(/\s*/g,"");
					
								window.location.href = "dingdan_xiang.html?danhao=" + _search_dan;
