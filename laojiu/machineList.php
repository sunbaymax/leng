...
<html>
	...
	<body>
		...
		<script>
			...
			if(!window.localStorage.getItem("isonlinelaojiu")) {
				window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxe215e9ec8c5c3306&redirect_uri=http://www.ccsc58.cc/leng/laojiu/oauth2_templatform.php&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect";
			}else{
...
//			}
			...
			function machine_ajax_list(_start) {
				...
				if(_userType == 'b') {
					$.ajax({
						url: "http://www.ccsc58.com/json/00_00_tb_shebeidongtai_laojiu.php",
						type: "post",
						data: {
...
//				});
			    	$.ajax({
					type: "post",
					url: "https://www.zjcoldcloud.com/xiandun/public/index.php/index/device_admin/laojiu_jiechu_debive",
					data: {
						deviceNumber: num
					},
					success: function(data) {
...
//				$(".window_post").removeClass('hidden');
	...
	$("#post_add_post").on("click", function() {
		...
		var _num = $(this).parents('.window_post').attr('num');
			$.ajax({
				type: "post",
				url: "https://www.zjcoldcloud.com/xiandun/public/index.php/index/device/update_beizhu",
				data: {
					devicenumber: _num,
					nickname: bieming
				},
