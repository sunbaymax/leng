jQuery(document).ready(function($) {
	if(window.localStorage.getItem("isonlinelaojiu")) {
		console.log(window.localStorage.getItem("isonlinelaojiu"),111)
	...
	$(".butok").on("click", function() {
		...
		var istel=isPhoneNumber(_userName);
				...
				success: function(data) {
					...
					if(_json.code == 1) {
						...
						if(_json.data.content.admin_user != '') {
							...
							window.localStorage.setItem("isonlinelaojiu", JSON.stringify(xduser))
							window.location.href = "../machineList.php?_r="+parseInt(Math.random()*10) ;
						}
