...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj, $timeOut = 600)
	{
	...
	public static function unifiedOrderForWeapp($inputObj, $timeOut = 600)
	{
		$url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet()) {
		...
		$inputObj->SetAppid('wx28db04fa5b729118');//公众账号ID
		$inputObj->SetMch_id('1454416802');//商户号
		$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip
		//$inputObj->SetSpbill_create_ip("1.1.1.1");
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	 ...
	 * @throws WxPayException
	 */
	public static function postXmlCurl($xml, $url, $useCert = false, $second = 600)
	{
		$ch = curl_init();
		//设置超时
