...
if (!$rs) {
	...
	$rs = $sth->fetch(PDO::FETCH_ASSOC);
	if (!$rs) {
		die("<script>alert('user device not exist')</script>");
	}
}
...
$rs = $sth->fetch(PDO::FETCH_ASSOC);
if (!$rs) {
	die("<script>alert('device not exist')</script>");
}

if ($rs['flow_type'] == '0') {
    die('<script>alert(\'该设备未初始化\')</script>');
}

if ($rs['flow_type'] == '2') {
    die('<script>alert(\'该套餐已是最高级\')</script>');
}

if ($rs['flow_type'] == $f_id) {
    die('<script>alert(\'不能升级同套餐\')</script>');
}
