...
$total = $rs['money'] * 100; //单位 分
//$total = 1;

$uid = isset($_GET['uid']) ? intval($_GET['uid']) : 0;
$sql = "select id from shield_user where id={$uid}";
$sth = $dbh->prepare($sql);
$sth->execute();
$rs = $sth->fetch(PDO::FETCH_ASSOC);
if (!$rs) {
...
$input->SetNotify_url("http://www.ccsc58.cc/leng/weixin/zhifu/example/freshdun_voicepay210301_notify.php");
			...
			WeixinJSBridge.invoke(
				...
				function(res) {
					...
					if(res.err_msg == "get_brand_wcpay_request:ok") {
                        ...
                        setTimeout(function () {
                            history.go(-1)
                            //window.location.href="../../../FreshShield/html/voicebill.html";
                        }, 1500);
