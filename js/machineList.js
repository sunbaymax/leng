$(document).ready(function() {
	...
	function myMachine() {
		...
		$(".content .list_center li a").on("click", function() {
			...
			window.location.href = "html/details_list.html?num_m=" + _val;
		})
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(_json.code == 30000) {
						...
						window.location.href = "html/details_list.html?num_m=" + _json.resultCode[0].shebeibianhao;
					}
				},
				error: function() {
					$(".wait").addClass("hidden");
		...
		function now_one_map_machine_list(_jingDu01, _weiDu01) {
			...
			geoc.getLocation(point, function(rs) {
				...
				if(address == "") {
					...
					_dem.find(".list_center li:nth-of-type(3) span").html(address);
				}
			})
		}
	}
